...
@click.option(
    "--type",
    "scope",
    type=click.Choice(["regional", "global"]),
    default="regional",
    show_default=True,
)
@click.option("--region")
...
@click.pass_obj
def cmd_create(
    cloudscale, ip_version, server_uuid, prefix_length, reverse_ptr, scope, region, tags
):
...
@click.pass_obj
def cmd_update(
    cloudscale, network_id, server_uuid, reverse_ptr, tags, clear_tags, clear_all_tags
):

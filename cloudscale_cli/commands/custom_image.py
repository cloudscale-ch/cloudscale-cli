...
@click.option(
    "--user-data-handling",
    type=click.Choice(["pass-through", "extend-cloud-config"]),
    required=True,
)
...
@click.pass_obj
def cmd_import(
    cloudscale, url, name, slug, user_data_handling, zones, source_format, tags, wait
):
    try:
        ...
        response = cloudscale.wait_for_status(
            uuid=response["uuid"],
            status="in_progress",
            max_sleep=8,
            retries=120,
...
@click.option("--slug")
@click.option(
    "--user-data-handling", type=click.Choice(["pass-through", "extend-cloud-config"])
)
@click.option("--tag", "tags", multiple=True)
...
@click.pass_obj
def cmd_update(
    cloudscale, uuid, name, slug, user_data_handling, tags, clear_tags, clear_all_tags
):
